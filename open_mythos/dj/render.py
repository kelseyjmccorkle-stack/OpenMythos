...
* ``soundfile``  — preferred audio I/O (falls back to the stdlib ``wave``
  module for 16-bit PCM mono output).
...
def _load_audio(path: str, sr: int):
    """Load ``path`` as mono float32 at ``sr``. Prefers soundfile, then librosa."""
    np = _require_numpy()
    try:
        import soundfile as sf  # type: ignore

        data, file_sr = sf.read(path, dtype="float32", always_2d=True)
        y = data.mean(axis=1)  # downmix to mono
        if file_sr != sr:
...
def _resample(y, src_sr: int, dst_sr: int):
    ...
    try:
        ...
        np = _require_numpy()
        # Linear-interpolation fallback (fine for our purposes).
        n_dst = int(round(len(y) * dst_sr / src_sr))
...
def _write_wav(path: str, y, sr: int) -> None:
    """Write mono float array to a 16-bit PCM WAV (soundfile if available)."""
    np = _require_numpy()
    y = np.clip(y, -1.0, 1.0)
    try:
        import soundfile as sf  # type: ignore
...
def render_mix(
    ...
    segments: list = []
    for step in steps:
        y = _load_audio(path_for(step.track), sr)
        if beatmatch and target_bpm and step.track.bpm > 0:
            y = _time_stretch(y, rate=step.track.bpm / target_bpm)
        cap = max_segment_seconds or step.cue_seconds or (len(y) / sr)
        y = y[: int(cap * sr)]
        segments.append(y)
    ...
    for i in range(1, len(segments)):
        ...
        nx = min(nx, len(out), len(seg) // 2 or len(seg))

        start = len(out) - nx  # incoming track overlaps the last nx samples
        ...
        head = seg[:nx] * fade_in
        out = np.concatenate([out[:-nx], tail + head, seg[nx:]])
