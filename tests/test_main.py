...
class TestGQAttention:
    ...
    def test_kv_cache_accumulates(self):
        cache = {}
        x = torch.randn(B, T, self.cfg.dim)
        # positions 0..T-1, then T..2T-1 — freqs must track the cache offset
        ...
        self.attn(x, self.freqs[T : 2 * T], kv_cache=cache, cache_key="layer0")
...
class TestMLAttention:
    ...
    def test_cache_accumulates_across_steps(self):
        ...
        self.attn(x, self.freqs[T : 2 * T], kv_cache=cache, cache_key="mla0")
